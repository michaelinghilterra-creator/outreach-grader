...
export function buildArgs(model, allowedTools) {
...
export function runClaudePrompt(prompt, { model, system, timeoutMs = 180000, allowedTools } = {}) {
  return new Promise((resolve, reject) => {
    const isWin = process.platform === 'win32';
    const args = buildArgs(model, allowedTools);
    ...
    child.on('close', (code) => {
      ...
      try {
        ...
        if (firstBrace !== -1 && lastBrace > firstBrace) {
          try { parsed = JSON.parse(out.slice(firstBrace, lastBrace + 1)); } catch { }
        }
      }
