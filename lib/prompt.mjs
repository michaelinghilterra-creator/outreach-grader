...
function renderResearchIntel(body, promptType) {
  const intel = body.research_intel;
  if (!intel || typeof intel !== 'object') return '';

  const contactSignals = Array.isArray(intel.contact_signals)
    ? intel.contact_signals.filter((signal) => typeof signal === 'string' && signal.trim())
    : [];
  const companySignals = Array.isArray(intel.company_signals)
    ? intel.company_signals.filter((signal) => typeof signal === 'string' && signal.trim())
    : [];
  const suggestedHooks = Array.isArray(intel.suggested_hooks)
    ? intel.suggested_hooks.filter((hook) => typeof hook === 'string' && hook.trim())
    : [];
  if (!contactSignals.length && !companySignals.length && !suggestedHooks.length) return '';

  const bullets = (items) => items.map((item) => `- ${item}`).join('\n');
  if (promptType === 'scoring') {
    return `RESEARCH CONDUCTED ON THIS RECIPIENT:

Contact signals:
${bullets(contactSignals)}

Company signals:
${bullets(companySignals)}

Suggested hooks:
${bullets(suggestedHooks)}

Use this intel to calibrate your scoring. Specifically:
- Personalization: if the message uses any of the signals above, score higher. If signals exist but the message ignores them, score lower and name the missed hook in top_fixes.
- Relevance: use the company signals to assess whether the sender's angle matches what this company is actually going through right now.
- Top fixes: if a suggested hook above is unused, include "Consider opening with: [hook text]" as one of the three fixes.

`;
  }

  return `AVAILABLE INTEL ON THE RECIPIENT (use this to improve the opener and personalization):
${bullets([...contactSignals, ...companySignals])}

Suggested hooks from the research:
${bullets(suggestedHooks)}

REWRITE RULES addition: incorporate at least one specific signal from the research into the opening hook. Do not use all of them. Pick the strongest one that fits the CTA goal.

`;
}

export function buildScoringSystemPrompt(body) {
...
8. SUBJECT LINE (weight: 10%)
  ...
  const researchIntel = renderResearchIntel(body, 'scoring');
...
${researchIntel}SCORING RUBRIC — score each dimension 1-10:
...
export function buildRewriteSystemPrompt(body) {
  const fixes = body.score_result.top_fixes.map((fix) => `- ${fix}`).join('\n');
  const researchIntel = renderResearchIntel(body, 'rewrite');
